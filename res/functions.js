	...
	if (common.length != '94') {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$.each(json.cards, function(i, cards) {

					//Ensure only expert cards from packs are added to each array
					if (cards.set == 'expert' && cards.collectible == true) {
						if (cards.quality == 'common') {
							common.push(cards.image);
						}
						if (cards.quality == 'rare') {
							rare.push(cards.image);
						}
						if (cards.quality == 'epic') {
							epic.push(cards.image);
						}
						if (cards.quality == 'legendary') {
							legendary.push(cards.image);
						}
					}
...
function getCards(rolls, common, rare, epic, legendary) {
	...
	drawCards(["legendary", "golden_epic", "legendary", "legendary", "rare"],["./res/mirrari_card_fixed_large.png", "./res/miracle_card_fixed_large.png", "./res/alesh_card_fixed_large.png", "./res/fake_card_fixed_large.png", "./res/trip_card_fixed.png"]);
};
